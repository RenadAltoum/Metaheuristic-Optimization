...
#Read one knapsack instance file and extracts profits and weights and capacity 
...
# Loads all files and return them as structured instances
...
def simulated_annealing_knapsack(
    ...
    # Initial random solution and its objective value 
    ...
    improvement_history = []  #Store (iteration, best_profit) when an improvement occurs
    # main cooling loop
    while temperature > minimum_temperature:
        ...
        for _ in range(iteration_limit):
            ...
            # generate a nighbor by fliping k distinct bits
            ...
            if delta > 0 or random.random() < math.exp(delta / max(temperature, 1e-12)):
                ...
                # Update best feasible if improved
        ...
        # adaptive tweak based Acceptance-rate  
...
# plots how the best profits improved over the search process
def plot_improvement_curve(history_data, output_path, chart_title):
    ...
    if not history_data:
        # save an empty placeholder plot when no improvenement occurred 
        ...
        plt.close()
        return
    # extract iteration numbers and corresponding best profits values
...
# RUN SA ON ALL FILES AND PLOT RESULTS
def run_batch_simulated_annealing(
    ...
    instances = load_all_instances(folder_path, file_pattern_suffix) #load all kn instances from the folder 
    ...
    for dataset_file, info in instances.items():
        ...
        for r in range(runs_per_file):
            ...
            if best_result is None or result["best_profit"] > best_result["best_profit"]: #keep the best one 
                best_result = result
        ...
        # store summary row for this instance 
...
if __name__ == "__main__":
    ...
    run_batch_simulated_annealing(
        ...
        warmup_factor=1.03,
        cooldown_factor=0.98,
        early_stop_no_improve=5,        # stop after 5 iteraction without improvment
        # output setting
        save_csv=True,
        csv_output_path="sa_results.csv",
        plots_folder="sa_plots",
